...
$E$-Feld + Ladungsträger\\
\textalign{$\hookrightarrow$}{
bewegte Ladungsträger\\
\textalign{$\hookrightarrow$}{
erzeugen Magnetfeld\\
\textalign{$\hookrightarrow$}{
beeinflusst bewegte Ladungsträger}}}

\textalign{$\Ra$}{
Zusammenhang/Wechselwirkung zwischen Elektrizität und Magnetismus\\
$\ra$ Induktion}
...
Magnetischer Fluss durch Spulenfläche (Anzahl der Feldlinien durch Fläche $A$)\\
\fbox{$\oint_{in} = \iint_A \vec{B} \circ d\vec{a}$} Integral über NICHT geschlossene Fläche\\
...
\subsection{\texorpdfstring{Möglichkeiten zur zeitlichen Änderung des magnetischen Flusses $\oint_m$}{Möglichkeiten zur zeitlichen Änderung des magnetischen Flusses}}
\begin{enumerate}
\item Flächenänderung
		\[A = A(t) \mbox{so.}\]
		\[\oint_m = \iint_A \vec{B} \circ d\vec{A} = B \iint_A da = BA\]
		\[\frac{d}{dt} \oint_m = B \mal l \mal \underbrace{\frac{d}{dt} x}_v\]

\item $B$-Feld-Änderung
		\[B = B(t)\]
		\[\mbox{Bsp. so, } v = 0\]
...
Lenzsche Regel:\\
\fbox{Induktionsstrom fließt stets so, dass er seiner Ursache entgegenwirkt}

Im Widerstand $R$ wird elektrische Arbeit (= Energie) verrichtet.
\[W_{el} = U_{ind} \mal I_{ind} \mal t\]
$\ra$ muss ''erzeugt'' werden\\
bewegtes Leiterstück wird von Induktionsstrom $I_{ind}$ durchflossen:\\
...
\textalign{$\ra$}{Energieerhaltungssatz erfüllt}}
...
Stromänderung $\frac{d}{dt} I$ in der Spule (vorgegeben)\\
\begin{itemize}[label=$\ra$]
\item Änderung des magnetischen Flusses $\oint_m$ in der Spule
\item Induktionsspannung in der Spule
		\[U_{ind} = -N \frac{d}{dt} \oint_m = -NA \frac{d}{dt} B(t)\]
\end{itemize}
$B$-Feld lange Spule (länge l):
\[B = \mu_0 \mu_r \frac{N}{l} I\]
\[U_{ind} = \underbrace{-\mu_0 \mu_r N^2 \frac{A}{l}}_{\mbox{Induktivität} L} \frac{d}{dt} I\]
Induktivität lange Spule:\\
\fbox{$L = \mu_0 \mu_r N^2 \dfrac{A}{l}$} $[L] = \frac{Vs m^2}{Am m} = \frac{Vs}{A} = H$ (Henry)\\
Maß dafür, wie ein zeitlich veränderlicher Strom in der Spule die Spulenspannung beeinflusst.

Schaltzeichen: 15.01.2008-IMG-phys-3

allgemein \fbox{$U = -L \dfrac{d}{dt} I$}

\textalign[Induktivität erzeugt Spannung die der Stromänderung entgegenwirkt]
{daher:}{
\begin{itemize}[label=\ra]
\item Induktivität ist ''bestrebt'' den Strom konstant zu halten
\end{itemize}
...
\item $U_{ind} \ra \infty$ bzw. wird extrem groß
\end{itemize}

für Gleichstrom: $\frac{d}{dt} I = 0$
\begin{itemize}[label=\ra]
\item $U = 0$
...
für Gleichspannung: $\frac{d}{dt} U = 0$
\begin{itemize}[label=\ra]
\item $I = 0$
\item Unterbrechung
\end{itemize}

für Wechselspannung: $\frac{d}{dt} U \ra \infty$ (hohe Frequenz)
\begin{itemize}[label=\ra]
\item $I \ra \infty$
\item Kurzschluss
\end{itemize}}
...
\bsp{Aufbau:}{
\underbrace{15.01.2008-IMG-phys-6}_{induktiv gekoppelt (über Magnetfeld)}

Wechselspannung: $U_p$ (z.B. Netzspannung, $230V$, $50Hz$)
\begin{itemize}[label=\ra]
\item Wechselstrom $I_p$ in Primärspule
\item magnetische Flussänderung in Primärspule
		$\approx$ (Eisenkerns) magnetische Flussänderung in Sekundärspule
		\begin{itemize}[label=\ra]
		\item Induktionsspannung $U_S$ in Sekundärspule
		\end{itemize}
\end{itemize}
\begin{align*}
...
\end{align*}
\fbox{$\dfrac{U_s}{U_p} = \dfrac{N_s}{N_p}$} ''Übersetzungsverhältnis''\\
