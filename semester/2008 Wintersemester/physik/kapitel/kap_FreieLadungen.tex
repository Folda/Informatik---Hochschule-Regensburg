...
Elektronen im Metall sind zwar beweglich, aber unterliegen dem Einfluss der positiven Atomrümpfe\\
$\ra$ Anziehung $\ra$ $e^-$ können Kristall nicht verlassen.\\
24.11.2008-IMG-phys-5\\
$e^-$ sammeln sich im Potenztopf (Coulombkräfte durch positive Atomrümpfe)\\
\textalign{$\ra$}{Energiezufuhr notwendig, um $e^-$ ins Vakuum zu befördern}
\renewcommand{\labelenumi}{\alph{enumi})}
\begin{enumerate}
\item $e^-$ soviel Energie durch Temperatur zuführen, dass sie ins Vakuum gelangen können (''Bindungskräfte aus Gitter überwinden'')\\
...
\item Feldemission\\
		...
		klassisch können $e^-$ Potentialwall nicht überwinden. Jedoch bei Wallbreite im atomaren Maßstab ($\ra$ hohes $e$-Feld)\\
		\textalign{$\ra$}{quantenmechanischen Tunneleffekt}\\
		\textalign{daher:}{$e^-$ können Potentialwall durchqueren (''durchtunneln'')}
		\textalign{$\ra$}{Erzeugung eines hohen elektrischen Feldes an Leiteroberfläche
		\[e \propto \frac{1}{r_{\mbox{oberfläche}}^2}\]}\\
		\textalign{$\Ra$}{Spritzen mit sehr kleinen Krümmungsradien (sehr spitz) $r = 10 \dots 100mmm$\\
		\textalign{$\ra$}{mikrotechnologisch herstellbar}}\\
		\textalign{$\ra$}{FED (Feldemissionsdisplay)\\
		für jedes Pixel ein Elektronenstrahl, also eine Spitze\\
		\textalign{Vorteile}{
		\begin{itemize}
		\item dünn (wie TFT, Plasma)
...
\textalign{$\Ra$}{\fbox{$v = \sqrt{\betrag{2 \frac{2}{m} U_{AB}}}$} (Betrag da $q < 0$, oder $U_{AB} < 0$ möglich)}\\
\textalign{Beispiel:}{Beschleunigung eines Elektrons\\
Elektron durchläuft Spannung von $U = 100V$\\
$m_e = \elekmass$\\
$q = - q_0 = - \elemlad$\\
...
\end{align*}
$\ra$ hohe Geschwindigkeit möglich\\
$\ra$ Gleichung nur gültig für $v \ll c$ (Lichtgeschwindigkeit)}}

\section{Elektronenstrahlröhre (Braunsche Röhre)}
