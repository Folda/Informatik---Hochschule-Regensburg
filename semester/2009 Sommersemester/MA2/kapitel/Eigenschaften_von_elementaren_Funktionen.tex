...
\fnewline
\begin{enumerate}[label=\roman*)]
\item bereits über \ac{Def.} der Ableitung gezeigt\\
		nun über Satz 4.10 \Todo{ref zu Satz 4.10}
		\begin{align*}
...
Die Funktion $f$ sei differenzierbar in $(0, 2)$ und es sei $f(0) = -3$ sowie $f'(x) \klgl 5$ für alle $x \in (0,2)$. Bestimmen Sie den größtmöglichen Wert für $f(2)$.

Lösung siehe \vref{sec:Eigenschaften_von_elementaren_Funktionen_A4_8L}.
...
Folgen Sie daraus eine Potenzreihendarstellung für $\frac{1}{(2 - y)^2}$

Lösung siehe \vref{sec:Eigenschaften_von_elementaren_Funktionen_A4_11L}.

\begin{fdefinition}[Natürlicher Logarithmus]
...
\begin{fsatz}[Eigenschaften des \ac{natürl.} Logarithmus]
\fnewline
\begin{enumerate}[label=\roman*)]
...
\item $\forall x > e^k \Ra \lnx > k$ ($k > 0$ beliebig)\\
		\ac{d.h.} jede beliebig große Schranke $k$ wird irgendwann überschritten und nie mehr unterschritten\\
		\Ra $\lim_{x \ra \un} \lnx = \un$
		\begin{itemize}
		\item $\lim_{x \searrow 0} \lnx = \lim_{y \ra \un} \rkl{\frac{1}{y}} = - \lim_{y \ra \un} \lnx[y] = - \un$
		\item für $\rkl{x_n}_{n \in \N}$ mit $x_n > 0$ und $\lim_{n \ra \un} x_n = \un$\\
				...
				* da $e^x$ schneller wächst als jede Potenz von $x$ (insbesondere schneller als $x^1 = x$)
		\end{itemize}
...
Lösung siehe \vref{sec:Eigenschaften_von_elementaren_Funktionen_A4_12L}.

\begin{fsatz}[Ableitung der Umkehrfunktion der trigonometrischen Funktionen]
\label{satz:Eigenschaften_von_elementaren_Funktionen_S4_16}
\fnewline
%\begin{itemize}[label=\roman*)]
%\item für $y \in \rkl{-1, 1}:~\frac{d}{dy} \arcsin (y) = \frac{1}{\sqrt{1 - y^2}}$
%\item für $y \in \rkl{-1, 1}:~\frac{d}{dy} \arccosx[y] = \frac{-1}{\sqrt{1 - y^2}}$
%\item für $y \in \R:~ \frac{d}{dy} \arctan (y) = \frac{1}{1 + y^2}$
%\end{itemize}
\Todo{Fix This}
\Mark{Satz 4.16}
\end{fsatz}
...
\fnewline
\begin{itemize}[label=\roman*)]
\item $f(x) = \sinx$ $f'(x) = \cosx$ $f^{-1} (y) = \arcsin(y)$
		\begin{align*}
		\frac{d}{dy} f^{-1} (y) &= \frac{1}{f'\rkl{f^{-1} (y)}} = \frac{1}{\cosx[\arcsin(y)]}\\
		&\arcsin (y) \in \rkl{- \frac{\pi}{2}, \frac{\pi}{2}} \tx{ und } \cosx \grgl 0 \tx{ für } x \in \rkl{-\frac{\pi}{2}, \frac{\pi}{2}} \Ra\\
		&= \frac{1}{\sqrt{\cos^2 \rkl{\arcsin(y)}}} = \frac{1}{\sqrt{1 - \sin^2 \rkl{\arcsin(y)}}} = \frac{1}{\sqrt{1 - y^2}}
		\end{align*}

\item analog zu i)
\item $\frac{d}{dy} \arctan (y)$ ($\frac{d}{dx} \tanx = 1 + \tan^2(x)$)
		\[= \frac{1}{1 + \tan^2 \rkl{\arctan (y)} = \frac{1}{1 + y^2}\] \qed
\end{itemize}
\end{fbeweis}

\begin{bemerkung}[zu Satz \vref{satz:Eigenschaften_von_elementaren_Funktionen_S4_16}]
\[\lim_{y \ra \un} \arctan (y) = \frac{\pi}{2}, \lim_{y \ra -\un} \arctan (y) = - \frac{\pi}{2}\]
\end{bemerkung}

\begin{fbeweis}
Wir wissen $\lim_{x \ra \frac{\pi}{2}} \tanx = \un$, $\lim_{x \ra -\frac{\pi}{2} \tanx = -\un$
\Insert{MA2-19.05.2009-IMG-1}
\begin{align*}
&\lim_{y \ra \un} \arctan (y) = \lim_{x \ra \frac{\pi}{2}} \arctan \rkl{\tanx} = \lim_{x \ra \frac{\pi}{2}} x = \frac{\pi}{2}\\
&\lim_{y \ra -\un} \arctan (y) = \lim{x \ra \frac{\pi}{2}} \arctan \rkl{\tanx} = \lim_{x \ra \frac{\pi}{2}} x = - \frac{\pi}{2}
\end{align*}
\end{fbeweis}
...
Lösung zu Aufgabe \vref{sec:Eigenschaften_von_elementaren_Funktionen_A4_8}.
\Insert{MA2-13.05.2009-IMG-1}
die Gerade $g$ liegt für $x \in \eklamm{0, 2}$ stets über der Funktion $f$ (\ac{bzw.} $f(x) \klgl g(x)$)\\
\Ra $f(2) \klgl g(2) = 5 \mal 2 - 3 = 7$

\subsection{Aufgabe 4.11}
\label{sec:Eigenschaften_von_elementaren_Funktionen_A4_11L}
Lösung zu Aufgabe \vref{sec:Eigenschaften_von_elementaren_Funktionen_A4_11}.
$\sum_{n = 1}^{\un} n \mal x^n \mal 1 = \frac{1}{(1 - x)^2}$\\
$\sum_{n = 1}^{\un} n \mal x^{n - 1} = \frac{0 \mal (1 - x) - 1 \mal (-1)}{(1 - x)^2} = \frac{1}{(1 - x)^2}$\\
Potenzreihendarstellung für $\frac{1}{(2 - y)^2}$
\begin{align*}
...
Lösung zu Aufgabe \vref{sec:Eigenschaften_von_elementaren_Funktionen_A4_12}.
\begin{enumerate}[label=\alph*)]
\item \begin{align*}
