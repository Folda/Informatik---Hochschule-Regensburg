\chapter{Höhere Ableitungen und Taylor-Entwicklung}
\Mark{Chapter 4.4}
\begin{fdefinition}
Die Funktion $f$ heißt $n$-mal differenzierbar falls $f' \frac{d}{dx} f$, $f'' \frac{d}{dx} f'$, $f''' \frac{d}{dx} f''$, $f^{(n)} \frac{d}{dx} f^{(n - 1)}$ existieren.
...
\end{enumerate}
zuerst 2. zeigen mit vollständiger Induktion (\ac{d.h.} falls \ggq{Ableitung} existiert hat sie diese Form)\\
...
Lösung siehe \vref{sec:Hoehere_Ableitungen_und_Taylor_Entwicklung_A4_13L}.

\section{Reihendarstellung}
Für eine oft differenzierbare Funktion hätte man gerne eine Darstellung durch eine Potenzreihe oder zumindest eine Näherung durch eine endliche Reihe.
...
Für eine $n$-mal differenzierbare Funktion $f$ heißt
...
Dann gilt für alle $x \in \eklamm{a, b}$ die Taylor-Formel
...
\begin{bemerkung}
Manchmal wird das zum Taylor-Polynom $n$-ter Ordnung gehörende Restglied auch mit $R_n$ (statt $R_{n + 1}$) bezeichnet.
\end{bemerkung}
...
\item $n = 2$
		...
		\end{align*}
		$T_2(f,x)$ ist eine quadratische Näherung der Funktion $f$, also eine Näherung durch eine Parabel.
\end{enumerate}

\begin{fdefinition}[Taylorreihe]
Ist eine Funktion beliebig oft differenzierbar, so heißt die Potenzreihe
...
\fnewline
\begin{enumerate}
\item Es gibt keine allgemeinen Aussagen über die Konvergenz von Taylorreihen (also ob ein Konvergenzradius $r > 0$ existiert und wie groß er gegebenenfalls ist).
\item Im Fall der Konvergenz der Taylorreihe (also innerhalb des Konvergenzgebietes) gilt nicht immer $f(x) = T(f, x)$.
\item Funktionen deren Taylorreihe konvergiert und für die im Konvergenzgebiet gilt $f(x) = T(f, x)$ nennt man \indexb{analytisch}. Viele gebräuchliche Funktionen sind analytisch: $e^x$, $\lnx$, $\sinx$, $\cosx$, $\tanx$, $\frac{1}{(1 - x)}$, $\rkl{\frac{1}{1 - x}}^2$, $\arcsin (x)$ \dots
\end{enumerate}
\end{bemerkung}
...
\item $f(x) = \lnx$, $x_0 = 1$\\
		...
		\end{align*}
		(könnte man mit vollständiger Induktion beweisen)
...
\begin{enumerate}
\item Bestimmen aller stationärer Punkte $x_1, x_2, \dots, x_e$
...
\item Ist $\max \rklamm{f(x_1), f(x_2), \dots, f(x_e), \lim_{x \searrow a} f(x), \lim_{x \nearrow b} f(x)} \in \eklamm{\lim_{x \searrow a} f(x), \lim_{x \nearrow b} f(x)}$ dann gibt es kein globales Maximum\\
		Andernfalls Maximalwert = $\max \rkl{f(x_1), f(x_2), \dots, f(x_e)}$ für Minimum analog\\
		\Insert{MA2-28.05.2009-IMG-1} hat kein globales Maximum auf Intervall $(0, 2)$ oder auch auf Intervall $(1, 2)$
		\begin{bemerkung}
		Es ist nicht immer nötig \ac{bzw.} nicht immer möglich $\lim_{x \searrow a} f(x)$, $\lim_{x \nearrow b} f(x)$ zu berechnen.
		\begin{enumerate}
		\item[nicht nötig] zum Beispiel $f$ hat nur in $x_1$ einen stationären Punkt. Wenn dann nur das globale Maximum gesucht ist und wir zeigen können, dass in $x_1$ ein lokales Maximum liegt ist in $x_0$ auch das globale Maximum.\\
		...
		\item[nicht möglich] \Insert{MA2-28.05.2009-IMG-3}
...
Finde den Punkt auf der Parabel $y^2 = 2x$ der am nähesten von $(1, 4)$ liegt.

Lösung siehe \vref{sec:Hoehere_Ableitung_und_Taylor_Entwicklung_A4_5L}.
...
Lösung zu Aufgabe \vref{sec:Hoehere_Ableitungen_und_Taylor_Entwicklung_A4_13}.
...
\item \begin{align*}
		...
		&= \begin{cases}\frac{2}{\sqrt{2 - x^2}} & \tx{für } x > 0\\\frac{-2}{\sqrt{2 - x^2}} & \tx{für } x < 0\end{cases}
		\end{align*}
...
Lösung zu Aufgabe \vref{sec:Hoehere_Ableitung_und_Taylor_Entwicklung_A4_5}.
...
& f'' (y) = 3y^2 > 0\\
\end{align*}
\Ra in $(2,2)$ ist das globale MInimum, also der Punkt der Parabel der am nächsten zu $(1,4)$ liegt.
