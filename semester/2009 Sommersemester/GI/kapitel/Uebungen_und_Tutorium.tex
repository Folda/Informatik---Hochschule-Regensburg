...
\item \begin{enumerate}[label=\arabic*)]
		\item ohne Gewicht
				\begin{itemize}
				\item Einträge in der Adjazenzmatrix: 1 oder 0
				\item Trennzeichen nicht nötig da Adjazenzmatrix quadratisch
						\begin{itemize}[label=$\hookrightarrow$]
						\item 2\,$\times$ durch String laufen nötig um Adjazenzmatrix zu rekonstruieren
						\end{itemize}
		...
		\item mit Gewicht
				\begin{itemize}
				\item in Adjazenzmatrix: Gewichte (\ac{z.B.} natürliche Zahlen)
...
\item Graph ohne Gewicht einfach alle Zeilen hintereinander schreiben
		\begin{itemize}[label=$\hookrightarrow$]
		\item neue Länge $n^2$ anstelle von $n^2 + n$
		\item Trennzeichen nicht nötig
		\end{itemize}
\end{enumerate}
...
\Todo{von meiner Lösung nachtragen}

\subsection{4. Aufgabe}
\Todo{von meiner Lösung nachtragen}

\subsection{5. Aufgabe}
unendliche Menge von Wörtern $y_1, y_2, \dots$ mit $\betrag{y_i} < \betrag{y_{i + 1}}$ für i = $1, 2, 3, \dots$ $\exists c$, so dass gilt
\[KC(y_i) \klgl \lceil \ld \ld \ld \betrag{y_i}\rceil + c\]
$y_i = 1^{2^{2^i}}$\\
$\betrag{y_i} = 2^{2^i}$, $\betrag{y_{i + 1}} = 2^{2^{i + 1}}$ \Ra $\underbrace{2^{2^i} < 2^{2^i}}_{\tx{für alle } i \in \mb{N}}$
