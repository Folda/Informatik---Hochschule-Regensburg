...
\item Man kann den Graphen \vref{fig:Uebung_GraphA1} in Form einer Adjazentenmatrix (Abbildung \vref{fig:Uebung_A1}) schreiben welche die Aussage enthält ($0$ ist nicht verbunden, $1$ ist verbunden) ob zwei Knoten über eine Kante miteinander verbunden sind.
		\begin{figure}[htb]
		\centering
...
\item $\gklamm{a}^* \gklamm{b}^* = \gklamm{a^i b^j \vert i, j \in \N}$
		...
		\Ra& y \in \gklamm{a}^k, z \in \gklamm{b}^m \tx{ für } \exists k, m \in \N\\
		\Ra& x = a^k b^m\\
...
\item $L_2 = \gklamm{w \in \Sigma_{\tx{Bool}}^* \vert \betrag{w}_0 = 4}$ - alle Binärwörter, die exakt 4 Nullen enthalten.\\
		Lösung siehe Abbildung \vref{fig:Uebung2_3a}.
		\begin{figure}[hbt]
		\centering
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,semithick]
...
\item $L_3 = \gklamm{w \in \Sigma_{\tx{Bool}}^* \vert 110 \tx{ ist kein Teilwort von } w}$
		Lösung siehe Abbildung \vref{fig:Uebung2_3b}.
		\begin{figure}[hbt]
		\centering
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,semithick]
...
\item $L_4 = \gklamm{w \in \Sigma_{\tx{Bool}}^* \vert \betrag{w} \klgl 5}$
		Lösung siehe Abbildung \vref{fig:Uebung2_3c}.
		\begin{figure}[hbt]
		\centering
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,semithick]
		\node[state,initial,initial text=,accepting] (n1) {};
