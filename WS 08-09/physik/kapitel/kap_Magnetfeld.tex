...
\textalign{genauer:}{magnetostatische Felder (daher. zeitlich nicht veränderbare Magnetfelder)}

\section{Eigenschaften von Magnetfeldern}
\textalign{Beobachtung:}{Magnete üben Kräfte aufeinander aus}
...
\item Richtung der Feldlinien gibt Richtung der Kraft an\\
		Dichte der Feldlinien gibt Stärke der Kraft/Feldes an
...
\item Nord- und Südpol treten stets gemeinsam auf
\item gleiche Pole stoßen sich ab
		18.12.2008-IMG-phys-1

\item ungleiche Pole ziehen sich an
		18.12.2008-IMG-phys-2

\item Feldlinien treten am Nordpol aus einem Magneten aus, und am Südpol wieder ein.
\item Größen:
		\begin{itemize}
		\item Magnetische Flussdichte $B$: Kraft- und Spannungsberechnung
		\item Magnetische Feldstärke $H$: Stromberechnung\\
				\fbox{$B = \mu H$}\\
		\end{itemize}
		$\mu$ Permeabilität (Materialkonstante)\\
		\fbox{$\mu = \mu_r \mu_0$}\\
		Vakuum: $\mu_r = 1$\\
...
\item Ursache für Magnetfelder:\\
		18.12.2008-DIA-phys-3\\
		Richtung des Magnetfelds:\\
		Rechte-Hand-Regel\\
		Däume in Richtung der technischen Stromrichtung, gekrümmter Finger in Richtung des Magnetfeldes

\item Stärke (Betrag) des Magnetfelds\\
		Durchflutungsgesetz\\
		...
		$I = \oint_{S} \vec{H} \mal d\vec{s} \underbrace{=}_{\vec{H} \vert \vert d \vec{s}} \oint_{S} H ds = \underbrace{=}_{H = \mbox{ const auf Kurve } S} \underbrace{H \oint_S ds}_{\mbox{Länge der Kurve}}$\\
		$I = H \mal 2 \pi r$\\
		...
		Vorgehensweise (analog zum Gaußschen Satz):
		\begin{itemize}
		\item Magnetfeld qualitativ zeichnen
		\item Kurve $S$ passend zur Feldgeometrie wählen $(\vec{H} \vert \vert d \vec{s})$
...
Ströme (bewegte Ladungen) verursachen ein Magnetfeld ($\equals$ Kraftwirkung)\\
...
\[F_{Lx} = F_{Ly} = 0\]
\fbox{$F_{LZ} = q \mal v_x \mal B_y$} für $\vec{v}$ senkrecht $\vec{B}$ TODO: senkrecht zeichen einfügen

\paragraph{Stromdurchflossener Leiter im homogenen Magnetfeld:}

$I = $ const. 22.12.2008-IMG-phys-2
\begin{itemize}
\item Ladung $q$ braucht Zeit $t = \frac{l}{v}$ um durch den Leiter zu fließen.
\item In der ZEit $t$ fliet eine Ladungsmenge $Q = I \mal t = I \mal \frac{l}{v}$ durch den Leiter.\\
		...
		\textalign{\fbox{$F = I \mal l_x \mal B_y$}}{für $\vec{l}$ senkrecht $\vec{B}$ TODO: senkrecht zeichen einfügen\\
		$\begin{array}{l}
		l_y = l_z = 0, l_x \neq 0\\
		B_x = B_z = 0, B_y \neq 0
		\end{array}$}
...
$\vec{F_L}$ senkrecht $\vec{v}$ TODO: senkrecht zeichen einfügen\\
%Arritemize
\begin{itemize}
\item Lorenzkraft kann nur die Richtung $\vec{v}$ ändern, nicht den Betrag.
...
$\ra$ größere Ablenkwinkel als bei elektro statischer Ablenkung
\begin{itemize}
\item CRT-Monitore
\item \textalign{Synchotron:}{geladene Teilchen auf Kreisbahn ($B$-Feld)\\
		Beschleunigung durch $E$-Feld}
\item REM (Bündelung von $e^-$-Strahlen und Ablenkung, um Oberfläche abzurastern)
\end{itemize}}
...
\item Ablenkung von $e^-$ auf eine Seite des Leiters\\
		%arritemize
		\begin{itemize}
		\item $\ra $ $e^-$ Überschuss $\ra$ negativ geladen\\
