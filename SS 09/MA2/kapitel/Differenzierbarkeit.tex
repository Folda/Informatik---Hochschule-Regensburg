...
Lösung siehe \vref{sec:Differenzierbarkeit_A4_1L}.
\Solved{Link zu Lösung}
...
\begin{fsatz}[Rechenregeln für Ableitungen]
Die Funktionen $f$ und $g$ seien im Punkt $x_0$ differenzierbar und $\alpha, \beta \in \R$. Dann gilt:
\begin{enumerate}[label=\roman*)]
\item Die Funktion $\alpha \mal f + \beta \mal g$ ist in $x_0$ differenzierbar und für die Ableitung gilt:
		\[\rkl{a \mal f + \beta \mal g}' (x_0) \rkl{=\frac{d}{dx} \rkl{\alpha \mal f (x) + \beta \mal g(x)}_{x = x_0}} = \alpha f'(x_0) + \beta \mal g'(x_0)\]

\item Die Funktion $f \mal g$ ist in $x_0$ differenzierbar und für die Ableitung gilt:
		\[(f \mal g)' (x_0) = f'(x_0) \mal g(x_0) + f(x_0) \mal g'(x_0)\]
		\indexb{Produktregel}

\item Falls $g(x_0) \neq 0$, ist die Funktion $\frac{f}{g}$ an der Stelle $x_0$ differenzierbar und für die Ableitung gilt die \indexb{Quotientenregel}
...
Lösung siehe \vref{sec:Differenzierbarkeit_A4_2L}.
...
Lösung siehe \vref{sec:Differenzierbarkeit_A4_4L}.
...
\item $h(u) = \cosx{u}^3$
\end{enumerate}
Hinweis: Sie dürfen verwenden
\begin{itemize}
\item $\forall r \in \R \backslash \gklamm{0}$: $\rkl{x^r}' = r \mal x^{r - 1}$
\item $\rkl{\cosx{x}}' = -\sinx{x}$
\end{itemize}

Lösung siehe \vref{sec:Differenzierbarkeit_A4_5L}.

\begin{fsatz}[Existenz und Ableitung der Umkehrfunktion]
Sei $f$: $\eklamm{a, b} \ra \R$ stetig und streng monoton wachsend (\ac{d.h.} $f(x) < f(\tilde{x}) für x < \tilde{x}$) mit $A \ceq f(a)$ und $B \ceq f(b)$ \ac{bzw.} stetig und streng monoton fallend (\ac{d.h.} $f(x) > f(\tilde{x})$ für $x < \tilde{x}$) mit $A \ceq f(b)$ und $B \ceq f(a)$. Dann existiert für $f$: $\eklamm{a, b} \ra \eklamm{A, B}$ eine Umkehrfunktion $f^{-1}$: $\eklamm{A, B} \ra \eklamm{a, b}$ mit
...
\begin{fsatz}[zu Satz 4.5]
\Todo{change to tofsatz}
...
\item $f^{-1} (f(x)) = x = \varphi(f(x))$ für $x \in \eklamm{a, b}$\\
		$1 = \frac{d}{dx} x_{x = x_0} = \frac{d}{dx} (\varphi(f(x)))_{x = x_0} = \varphi ' (f(x_0)) \mal f'(x_0)$\\
		\Ra $\varphi '(f(x_0)) = \rkl{f^{-1}} (f(x_0)) = \frac{1}{f' (x_0)}$
\item $f (f^{-1} (y)) = y = f(\varphi(y))$ für $y \in \eklamm{A, B}$\\
		2. Aussage folgt analog aus \textcircled{2}
\end{enumerate}
\end{fbeweis}

\begin{bemerkung}
Betrachtet man eine Funktion der Zeit \ac{z.B.} $f(t)$ \ac{bzw.} $x(t)$ mit $t \stack{^}{=} \tx{Zeit}$, schreibt man statt $\frac{df}{dt} = f$ \ac{bzw.} $\frac{dx}{dt} (t)_{t = t_0} = x (t_0)$
\end{bemerkung}
...
\end{align*}
mit Formel aus Satz 4.5 \Todo{ref zu Satz 4.5} folgt
\begin{align*}
...
Lösung siehe \vref{sec:Differenzierbarkeit_A4_6L}.
...
\ac{d.h.} es existiert ein $x_0 \in (a, b)$ mit $f(x_0) \neq f(a)$ \ac{OBdA} $f(x_0) > f(a)$, \ac{d.h.} da Maximum von $f$ wird nicht am Rand von $\eklamm{a, b}$ angenommen, sondern im Inneren, also $\overline{x} \in (a, b)$ (siehe Satz 3.7) \Todo{Ref zu Satz 3.7} \ac{d.h.} $f(\overline{x}) \grgl f(x)$ $\forall x \in \eklamm{a, b}$\\
Nach Satz 4.6 \Todo{Ref zu Satz 4.6} gilt $f'(\overline{x}) = 0$
\[\xi \ceq \overline{x}\]
\end{description}
\end{fbeweis}
...
\begin{itemize}
\item Nach Satz 3.7 \Todo{Ref zu Satz 3.7} wird Minimum angenommen.
\item $f(0) = 0 = f(1)$ \ac{d.h.} nach Satz 4.7 \Todo{Ref zu Satz 4.7} \ac{ex.} $\xi \in (0, 1)$ mit $f'(\xi) = 0$
\item da $f\rkl{\frac{1}{2}} = \rkl{\frac{1}{2}}^3 - \rkl{\frac{1}{2}}^2 = \frac{1}{8} - \frac{1}{4} = - \frac{1}{8} < 0$ ist das Minimum nicht am Rand
		\begin{align*}
		f'(x) &= 3x^2 - 2x \stack{!}{=} 0\\
		&\Lra x(3x - 2) \stack{!}{=} 0\\
		&\Lra x = 0 \oder 3x - 2 = 0\\
		&\Lra \ubl[10mm]{x = 0}{keine mögliche Minimalstelle, da $f \rkl{\frac{1}{2} < f(0)}$} \oder x = \frac{2}{3}\\
		\end{align*}
...
Von einem rechteckigen Stück Pappe mit den Seitenlängen $a = 15cm$ und $b = 24cm$ wird an jeder Ecke ein Quadrat mit der Seitenlänge $x$ weggeschnitten. Auffalten der vorstehenden Rechtecke entsteht eine nach oben offene Schachtel. Für welches $x$ hat die Schachtel das maximales Volumen.

Lösung siehe \vref{sec:Differenzierbarkeit_A4_7L}.
...
Lösung zu Aufgabe \vref{sec:Differenzierbarkeit_A4_1}.
...
Lösung zu Aufgabe \vref{sec:Differenzierbarkeit_A4_2}.
\begin{enumerate}[label=\alph*)]
\item $f_1(x) = x^{45} + x^{-45}$
		\begin{align*}
...
Lösung zu Aufgabe \vref{sec:Differenzierbarkeit_A4_4}.
\begin{enumerate}[label=\alph*)]
\item $\frac{d}{dx} \rkl{\frac{x^2}{f(x)}}_{x = 2}$
		\begin{align*}
...
Lösung zu Aufgabe \vref{sec:Differenzierbarkeit_A4_5}.
\begin{enumerate}[label=\alph*)]
\item $h(x) = 4(5 - 2x)^3$
...
\item $g(t) = \frac{1}{\sqrt{2t +4}} = \sqrt{\frac{1}{2t + 4}} = \rkl{\rac{1}{2t + 4}}^{\frac{1}{2}}$
		\begin{enumerate}[\textcircled{\arabic*}]
...
Lösung zu Aufgabe \vref{sec:Differenzierbarkeit_A4_6}.
...
\end{align*}
\[\rkl{f^{-1}}' \tx{ mit Satz 4.5}\]
\Todo{Ref zu Satz 4.5}
\begin{align*}
...
Lösung zu Aufgabe \vref{sec:Differenzierbarkeit_A4_7}.
...
\end{align*}
$10$ ist keine mögliche Lösung, da dann eine negative Breite auftauchen würde.\\
$f(0) = 0 = f(7,5)$ und für $x = 1$ $f(1) > 0$ \Ra in 3 Maximum
