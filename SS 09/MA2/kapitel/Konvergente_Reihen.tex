...
Für alle $q \in \mb{R} \backslash \gklamm{1}$ und $n \in \mb{N}_0$ gilt:
...
%MARK: Definition 2.9
Sei die Folge $(A_k)_{k \in \mb{N}}$ gegeben. Die Folge $(S_n)_{n \in \mb{N}}$ der \indexu{Partialsummen} ist definiert durch
\[S_n \ceq \sum_{k = 1}^n A_k\]
Man nennt $(S_n)_{n \in \mb{N}}$ auch \indexu{unendliche Reihe} mit den Gliedern $A_k$ (geschrieben $\sum_{k = 1}^{\infty} A_k$ oder $\sum A_k$).\\
...
\begin{bemerkung}
Unendliche Reihen können bei einem beliebigen Index $k \in \mb{Z}$ beginnen ($\sum_{k = j}^{\infty} A_k$) meistens $j = 0$ oder $j = 1$.
\end{bemerkung}

\begin{fsatz}[Unendliche geometrische Reihe]
Für die (unendliche) geometrische Reihe gilt:
\[\sum_{k = 0}^{\infty} q^k \text{ ist divergent für } \betrag{q} \grgl 1\]
\[S \ceq \sum_{k = 0}^{\infty} q^k = \frac{1}{1 - q} \tx{ für } \betrag{q} < 1\]
\end{fsatz}
...
\item[Fall 2] $\betrag{q} > 1$
		...
		ist nicht beschränkt, da $q^{n + 1}$ für $\betrag{q} > 1$ divergent
...
\item[Fall 4] $q = -1$
		\[S_n = \sum_{k = 0}^n (-1)^k = \begin{cases}1 \text{ für } n \tx{ gerade}\\0 \tx{ für } n \tx{ ungerade}\end{cases}\]
		\begin{itemize}[label=\Ra]
		\item $(S_n)_{n \in \mb{N}}$ divergent
		\item $\sum_{k = 0}^{\infty} A_k$ divergent
		\end{itemize}
...
\begin{fsatz}[Konvergenzkriterien für Reihen]
\mbox{}\par
\begin{enumerate}[label=\roman*)]
\item Notwendiges Kriterium\\
		\[\sum_{k = 0}^{\infty} A_k \tx{ konvergent } \Ra \lim_{k \to \infty} A_k = 0\]
		Dies ist aber kein zwingendes Kriterium für konvergenz eher für das Beweisen der divergenz geeignet.

\item Leibnitz-Kriterium (für alternierende Reihen)\\
...
\item Quotientenkriterium
		\[\lim_{k \ra \infty} \betrag{\frac{A_k + 1}{A_k}} = q = \begin{cases}< 1 & \Ra \sum_{k = 0}^{\infty} A_k, \sum_{k = 0}^{\infty} \betrag{A_k} \tx{ konvergent}\\> 1 & \Ra \sum_{k = 0}^{\infty} A_k, \sum_{k = 0}^{\infty} \betrag{A_k} \tx{ divergent}\end{cases}\]
\end{enumerate}
\end{fsatz}
...
\item Kontraposition \ac{z.z.}
		...
		\draw[decorate] (5.3,-0.2) -- (3.5,-0.2) node[below,midway] {mögl. positionen};
		\draw[decorate] (4.8,-1) -- (1.5,-1) node[below,midway] {mögl. positionen};
		\end{tikzpicture}
...
\item $A_{k + 1} \klgl A_k$ $S_n = \sum_{k = 1}^n (- 1)^k A_k$
		...
		\[\underbrace{S_1 \klgl S_3 \klgl S_5 \klgl \dots \klgl S_{2n + 1}}_{\begin{minipage}{35mm}\begin{center}\scriptsize{monoton wachsende $n$ nach oben beschränkt Folge mit $T \ceq \lim_{n \ra \infty} s_{2n + 1}$}\end{center}\end{minipage}} \klgl S \klgl \underbrace{S_{2n} \klgl \dots \klgl S_6 \klgl S_4 \klgl S_2}_{\begin{minipage}{32mm}\begin{center}\scriptsize{monoton fallende und nach unten beschränkte Folge mit $U \ceq \lim_{n \ra \infty} s_{2n}$}\end{center}\end{minipage}}\]
		\begin{align*}
		U - T &= \lim_{n \ra \infty} \rklamm{S_{2n}} - \lim_{n \ra \infty} \rklamm{S_{2 n + 1}} = \lim_{n \ra \infty} \rklamm{S_{2n} - S_{2n + 1}}\\
		&= \lim_{n \ra \infty} \rklamm{\sum_{k = 1}^{2n} (-1)^k A_k - \sum_{k = 1}^{2n + 1} (-1)^k A_k} = \lim_{n \ra \infty} -(-1)^{2n + 1} \mal A_{2n + 1} = \lim_{n \ra \infty} A_{2n + 1} = 0\\
...
\item Wurzelkriterium
		...
		\item[Fall 2] $w > 1$
				...
				\Ra $\sum \betrag{A_k}$ divergent (Rechnug für $\sum_{A_k}$ analog)
		\end{description}

\item Beweis Quotientenkriterium ähnlich wie Wurzelkriterium \qed
\end{enumerate}
\end{fbeweis}

\begin{fsatz}[Cauchycher Verdichtungssatz]
...
\item Verallgemeinerte harmonische Reihe
		...
		\begin{itemize}
		\item divergent für $\alpha \klgl 1$ (da $^{1 - \alpha} \grgl 1$ und geometrische Reihe dafür divergent)
		\item konvergent für $\alpha > 1$ (da $2^{1 - \alpha} < 1$ und geometrische Reihe für $q < 1$ konvergent)
		\end{itemize}
...
Lösung siehe \vref{sec:Konvergente_Reihen_A2_3L}.

\section{Aufgabe 2.4}
\label{sec:Konvergente_Reihen_A2_4}
Für welche Werte von $r$ konvergiert die Reihe
\[1+2r+r^2+2r^3+r^4+2r^5+r^6+\dots\]
Bestimmen Sie im Fall der Konvergenz den Grenzwert.

Lösung siehe \vref{sec:Konvergente_Reihen_A2_4L}.
...
Lösung zu Aufgabe \vref{sec:Konvergente_Reihen_A2_3}.
...
\item $\sum_{n = 1}^{\infty} \frac{1}{2+ 3^n}$
		...
		\item Majorantenkriterium
				\[\sum_{n = 1}^{\infty} \frac{1}{2 + 3^n} \klgl \underbrace{\sum_{n = 1}^{\infty} \rklamm{\frac{1}{3}}^n}_{\begin{minipage}{10mm}\begin{center}\scriptsize{konvergente geometrische Reihe}\end{center}\end{minipage}} \Ra \tx{ konvergent}\]
		...
		\item Quotientenkriterium
				...
				&= \lim_{n \ra \infty} \frac{\frac{2}{3^n} + 1}{\frac{2}{3^n} + 3} = \frac{\lim_{n \ra \infty} \rklamm{\frac{2}{3^n} + 1}}{\lim_{n \ra \infty} \rklamm{\frac{2}{3^n} + 3}} = \frac{1}{3} < \Ra \tx{ konvergent}
				\end{align*}

		\item Wurzelkriterium
				\[0 \klgl \lim_{n \ra \infty} \sqrt[n]{\betrag{A_n}} = \lim_{n \ra \infty} \sqrt[n]{\betrag{\frac{1}{2 + 3^n}}} \klgl \lim_{n \ra \infty} \sqrt[n]{\frac{1}{3^n}} = \lim_{n \ra \infty} \frac{1}{3} = \frac{1}{3} \Ra \tx{ konvergent}\]

		\item Verdichtungssatz
				\[\sum_{n = 0}^{\infty} 2^n \mal \frac{1}{\underbrace{2 + 3^{2^n}}_{= A_{2^n}}} \klgl \sum_{n = 0}^{\infty} 2^n \mal \rklamm{\frac{1}{3^2}}^n = \underbrace{\sum_{n = 0}^{\infty} \rklamm{\frac{2}{9}}^n}_{\tx{konvergente geometrische Reihe}}\]
...
Lösung zu Aufgabe \vref{sec:Konvergente_Reihen_A2_4}.
