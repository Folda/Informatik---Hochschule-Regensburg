...
nicht regulär.\\
$L_{\sharp}$ erfüllt die Pumping-Eigenschaft des verschärften \ac{PL}. Sei $z \in L_{\sharp}$, dann $z \in \gklamm{\sharp}^+ L$ oder $z \in \Sigs$
\begin{enumerate}
...
\item Sei $\betrag{z} = n$ mit $n \grgl n_0$ und gelte $z' \in L \Lra z' \in L(M)$ für alle $z'$ mit $\betrag{z'} < n$. Angenommen $z = z_0 a s_0$ mit $\betrag{z_0 a} = n_0$.\\
		Gemäß Konstruktion von $\delta$ (2. Fall) \Todo{Ref zur definition} ist
		...
		also:
		\[u w s_0 \in L \stack{\tx{\ac{IV}}}{\Lra} u w s_0 \in L(M) \stack{\tx{Konstruktion von } \delta}{\Lra} z =z_0 a s_0 \in L(M)\]
		\Todo{Make longstack command}
\end{itemize}
\end{fbeweis}
...
\begin{itemize}
\item $r = a$ mit $a \in \Sigma$ \Insert{GI-26.05.2009-IMG-1}
\item $r = \epsilon$ \Insert{GI-26.05.2009-IMG-2}
\item $r = \emptyset$ \Insert{GI-26.05.2009-IMG-3}
\item $r = \rkl{r_1 \mal r_2}$ Laut \ac{IV} \ac{ex.} $\epsilon$-\ac{NEA}, die $L(r_1)$ und $L(r_2)$ erkennen (siehe Abschlusseigenschaft)
		\Insert{GI-26.05.2009-IMG-4}

\item $r = \rkl{r_1 + r_2}$ \ac{IV} gilt wie vor.
		\Insert{GI-26.05.2009-IMG-5}

\item $r = \rkl{r_1}^*$
		\Insert{GI-26.05.2009-IMG-6}
...
\begin{beispiel}
\ac{DEA}: \Insert{GI-26.05.2009-IMG-7}
\ac{G-NEA}: \Insert{GI-26.05.2009-IMG-8}
\begin{idee}
Sukzessives Eliminieren von Zuständen / Kompensation über Kante mit \ac{RA}.
\end{idee}
\Insert{GI-26.05.2009-IMG-9}

Löschung von Knoten 1:
\Insert{GI-26.05.2009-IMG-10}

Löschung von Knoten 2:
\Insert{GI-26.05.2009-IMG-11}
...
\item Gewinne regulären Ausdruck für $L (M)$ wie folgt:
		...
		\item Setze $M' = M''$ und fahre bei a) fort. \Todo{Ref zu a)}
		\end{enumerate}
\end{enumerate}
\end{beispiel}
...
\node[state] (20) [right of=11] {20};

\tikzedge{P}{10}{a}{}{};
\tikzedge{10}{11}{v}{}{};
\tikzedge{11}{20}{a}{}{};

\tikzedge{10}{P}{r}{bend right}{};
\tikzedge{20}{10}{r}{bend left}{};

\node (text) [right of=20, node distance=2.5cm] {
\begin{minipage}{22.88mm}
$a$-Ausleihen\\
$v$-Vorbestellen\\
$r$-Rückgabe
\end{minipage}};
...
\tikzedge{11P}{20P}{a.A}{}{};
\path[->, dashed, color=red] (11P) edge node {$a.B$} (1110);
