...
\item[Algorithmus] $A: \Sigma_{\tx{Input}}^* \ra \Sigma_{\tx{Output}}^*$\\
		...
		\begin{itemize}
		\item Eingabewörter schrittweise verarbeitet (deterministisch).
		...
		\end{itemize}
\end{description}
Wozu formales Modell, wenn wir Algorithmen auch so (\ac{z.B.} Programmiersprache) beschreiben können?
\begin{itemize}
\item geht es um Berechenbarkeit reicht intuitives Vorgehen für positive Antwort aus, da Programmier Sprachen eindeutige Semantik hat
...
\end{itemize}
$\delta(q, a) = \rkl{q_1, b, L}$ bedeutet, dass die \ac{TM} im Zustand $q$ durch Lesen des Symbols $a$ in den Zustand $q_1$ übergehen kann, das Symbole $a$ durch $b$ ersetzt und den \ac{S-/L-Kopf} eine Position nach links bewegt.
\Mark{Definition 5.1}
\end{fdefinition}
\begin{fdefinition}
...
Für ein Wort $w \in \Sigs$ ist $q_0 \cent w$ eine Startkonfiguration; eine Konfiguration der Gestalt $v_1 q_{\tx{accept}} v_2$ heißt akzeptierende Konfiguration und eine Konfiguration der Gestalt $v_1 q_{\tx{reject}} v_2$ heißt verwerfende Konfiguration $\rkl{v_1, v_2 \in \Gamma^*}$. Eine Konfiguration $v_1 q a v_2$ mit $v_1, v_2 \in \Gamma^*$, $a \in \Gamma$, $q \in Q$ beschreibt folgende Situation.
\begin{itemize}
...
\item der \ac{S-/L-Kopf} befindet sich auf dem $\rkl{\betrag{v_1} + 1}$-ten Feld des Bandes und liefert $a$ als nächstes Symbol.
\end{itemize}
\Mark{Definition 5.2}
\end{fdefinition}
\begin{fdefinition}
Sei $M = \rkl{Q, \Sigma, \Gamma, \delta, q_0, \qacc, \qrej}$ eine \ac{TM}. Ein Konfigurationsübergang (ein Schritt) von $M$ ist eine Relation $\vd:~\Konf{(M)} \times \Konf{(M)}$, die \ac{def.} ist durch
\begin{enumerate}
...
\end{fdefinition}
\begin{fdefinition}
Sei $M = \rkl{Q, \Sigma, \Gamma, q_0, \qacc, \qrej}$ eine \ac{TM} und $w \in \Sigs$. Eine Berechnung ist eine, möglicherweise unendliche, Kette von Konfigurationen:
\[C = C_0 C_1 \dots\]
mit $C_i \in \Konf{(M)}$ und $C_i \vd_M C_{i + 1}$ ($i \in \N$). Ist die Berechnung endlich so endet sie in einer Konfiguration $C_n = v_1 q v_2$ mit $q \in \gklamm{\qacc, \qrej}$ und $\ubl[\widthof{\tx{beginnen}}]{v_1}{muss mit $\cent$ beginnen}, v_2 \in \Gamma^*$. Eine Berechnung von $M$ heißt
\begin{itemize}
...
Wir sagen, dass $M$ eine Funktion $f: \Sigs \ra \Gamma^*$ berechnet, falls für alle $x \in \Def{(f)}$
...
Indikatoren für Korrektheit dieser These
\begin{itemize}
...
\item andere Zugänge zum Berechenbarkeitsbegriff ($\lambda$-Kalkül, $\mu$-rekursive Funktionen) sind äquivalent
\item alle modernen Programmiersprachen lassen sich auf \ac{TM} reduzieren
\end{itemize}
\Mark{Satz 5.1}
\end{fsatz}
\begin{fdefinition}
Eine Sprache $L$ heißt rekursiv aufzählbar, wenn eine \ac{TM} $M$ mit $L(M) = L$ existiert.
\[\mathcal{L}_{\tx{RE}} = \gklamm{L(M) \vert M \tx{ ist \ac{TM}}}\]
ist die Menge aller rekursiv aufzählbarer Sprachen.
\Mark{Definition 5.6}
\end{fdefinition}
\begin{fdefinition}
Eine Sprache $L$ heißt \textbf{rekursiv} (entscheidbar), falls eine \ac{TM} $M$ existiert, so dass $\forall w \in \Sigs$ gilt:
\begin{itemize}
\item $q_0 \cent w \vd^* u \qacc v$, falls $w \in L$
\item $q_0 \cent w \vd^* u \qrej v$, fa,,s $w \notin L$
\end{itemize}
mit $u \in \Gamma^*$, $v \in \Gamma^+$.
\[\mathcal{L}_{\tx{R}} = \gklamm{L (M) \vert M \tx{ ist \ac{TM}, die immer hält}}\]
...
\begin{ziel}
Nachweis der Existenz von Sprachen, die nicht rekursiv aufzählbar sind.
\end{ziel}
Um dies zu zeigen werden wir folgende quantitative äquivalente Aussage beweisen.
\begin{quote}
\ggq{Die Mächtigkeit der Menge aller \ac{TM} ist kleiner als die Mächtigkeit aller Sprachen über $\SigB$.}
\end{quote}
...
\begin{multicols}{2}
\begin{itemize}
\item $\Code{(q_i)} = 1 0^i 1$ für $i = 1 \dots m$
