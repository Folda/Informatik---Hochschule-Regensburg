...
Gegeben sei das ALphabet $\Sigma = \gklamm{0, 1}$. Ferner seien $L_1 = \gklamm{0, 00, 000}$ und $L_1 = \gklamm{0, 1, 01}$. Für welche Sprachen stehen die folgenden Ausdrücke?
...
Geben Sie induktive Definitionen für die folgenden Funktionen an:
\begin{enumerate}[label=(\alph*)]
\item $\del$: $\Sigma^* \times \Sigma \ra \Sigma^*$ entfernt alle Vorkommen eines Symbols aus einem Wort (z.B. $\del(abbcab, a) = bbcb$).
\item $\gicount$: $\Sigma^* \times \Sigma \ra \mb{N}$ zählt alle Vorkommen eines Symbols in einem Wort (z.B. $\gicount$ ist $count(abbdd, d) = 2$).
\end{enumerate}

\subsection{3. Aufgabe (3+3+3 Punkte)}
Gegeben sei das Alphabet $\Sigma = \gklamm{0, 1, \sharp}$.
\begin{enumerate}[label=(\alph*)]
\item Wie viele verschiedene Wörter der Länge $n$gibt es?
\item Wie viele verschiedene Wörter der Länge $n$ gibt es, in denen ein Symbol $c \in \Sigma$ genau $k$-mal vorkommt?
\item Wie viele verschiedene Wörter der Länge $n$ gibt es, in denen ein Symbol $c \in \Sigma$ mindestens $k$-mal vorkommt?
\end{enumerate}
...
Dabei dürfen $L_1$ und $L_2$ weder gleich, noch leer sein und müssen sich von $\gklamm{0, 1}^*$ unterscheiden. Beweisen Sie ihre Aussage.
...
$v = a_1, a_2 \dots a_k (1 \klgl i \klgl k)$ für $k \in \mb{N}$, $a_i \in L_1 \cup L_2 = \gklamm{0, 01, 00}$\\
$u = a_2 \dots a_k$
\Ra $v \in L_0$\\
